    ...
    const ADVANCING_TYPES: WorkoutType[] = ["A", "B", "C", "Cardio"];
...
function getLastCycleStep(logs: WorkoutLog[]): CycleStep | null {
  ...
  for (const log of sorted) {
    // Skip entries that don't advance the cycle
    ...
    if (log.workout_type === "A") {
      ...
      // Only advance through rest steps if manually marked
        ...
        <div className="bg-surface border border-border rounded-2xl p-4">
          ...
          <div className="mb-3">
            <div className="flex justify-between text-xs text-muted mb-1.5">
              ...
              <span>
                {lastCycleStep ? `${nextIdx} / ${CYCLE_SEQUENCE.length} done` : "Starting fresh"}
              </span>
            </div>

...
function wasActivityLoggedToday(logs: WorkoutLog[]): boolean {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
export default function NextWorkout({
  ...
  logs: WorkoutLog[];
  nextStepOverride?: CycleStep;
}) {
  const nextStep = nextStepOverride ?? getNextCycleStepFromLogs(logs);
  const meta = STEP_META[nextStep];
  const nextIdx = CYCLE_SEQUENCE.indexOf(nextStep);
  const daysSinceLast = getDaysSince(logs);
  const loggedToday = wasActivityLoggedToday(logs);
